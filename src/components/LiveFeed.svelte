<script lang="ts">
  ...
  onMount(() => {
    // Check if the camera iframe loads successfully — use load/error events
    // instead of fetch() which causes a CORS error for cross-origin streams
    const liveFeed = document.getElementById('live-feed') as HTMLIFrameElement;

    const checkFeed = () => {
      if (!liveFeed) return;
      try {
        // If the iframe has content, show it; otherwise hide behind no-signal
        const hasContent = liveFeed.contentDocument && liveFeed.contentDocument.body &&
          liveFeed.contentDocument.body.innerHTML.length > 0;
        liveFeed.style.zIndex = hasContent ? '20' : '0';
      } catch {
        // Cross-origin — can't read contentDocument, just show it and let the browser handle
    ...
    setInterval(checkFeed, 5000);
    window.addEventListener('resize', adjustVideoSize);

    // Check production flag via heartbeat (POST)
    fetch('/api/mavlink/heartbeat', { method: 'POST', headers: { 'Content-Type': 'application/json' } })
      .then((res) => {
        if (res.ok) isProduction = res.headers.get('isProduction') === 'true';
      })
...
>
  <div class="container w-full h-full relative">
    ...
    <button class="absolute top-2 right-14 p-2 px-[14px] rounded-full z-20 opacity-[60%]" on:click={rotateVideo}>
